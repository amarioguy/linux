				      ...
				      size_t cal_blob_size)
{
	...
	blocks_left = DIV_ROUND_UP(left, BCM4378_CALIBRATION_CHUNK_SIZE) - 1;

	while (left) {
		size_t transfer_len =
			min(left, (size_t)BCM4378_CALIBRATION_CHUNK_SIZE);

		ret = __bcm4378_send_calibration_chunk(
			bcm4377, cal_blob + done, transfer_len, blocks_left);
		if (ret) {
			dev_err(&bcm4377->pdev->dev,
						   ...
						   const char *suffix)
{
	...
	snprintf(name0, sizeof(name0), "brcm/brcmbt%s%s-%s-%s.%s",
		 bcm4377->hw->name, bcm4377->stepping, bcm4377->board_type,
		 bcm4377->vendor, suffix);
	snprintf(name1, sizeof(name1), "brcm/brcmbt%s%s-%s.%s",
		 bcm4377->hw->name, bcm4377->stepping, bcm4377->board_type,
		 suffix);
			    ...
			    const struct firmware *fw)
{
	...
	for (i = 0, left = fw->size; i < chunks; ++i, left -= transfer_len) {
		transfer_len = min(left, (size_t)BCM4378_PTB_CHUNK_SIZE);
